...
// Enqueue plugin styles with high priority for Elementor compatibility
function salat_times_enqueue_styles() {
    wp_enqueue_style('salat-times-styles', plugins_url('css/salat-times.css', __FILE__), array(), '3.7', 'all');
    
    // Add inline CSS for Elementor editor
    if (did_action('elementor/loaded')) {
        wp_add_inline_style('salat-times-styles', '
            .elementor-editor-active .salat-times-container,
            .elementor-editor-preview .salat-times-container {
                pointer-events: none;
            }
            .elementor-editor-active .salat-times-widget,
            .elementor-editor-preview .salat-times-widget {
                min-height: 50px;
            }
        ');
    }
}
add_action('wp_enqueue_scripts', 'salat_times_enqueue_styles', 100);
...
function add_elementor_widget_categories($elements_manager) {
    $elements_manager->add_category(
        'salat-times',
        [
            'title' => __('Salat Times', 'salat-times'),
            'icon' => 'fa fa-clock',
        ]
    );
}
...
function register_salat_times_widget($widgets_manager) {
    if (class_exists('\\Elementor\\Widget_Base')) {
        require_once(__DIR__ . '/widgets/elementor-salat-times-widget.php');
        $widgets_manager->register(new \Elementor_Salat_Times_Widget());
    }
}
add_action('elementor/widgets/register', 'register_salat_times_widget');
...
function daily_salat_times() {
	...
	$default_options = array(
		...
		'talign' => 'center',
		'walign' => 'left',
		'scheme' => '#6c1638 #ffffff #6c1638 #ffffff #e3b7c7 #f1dbe3 #000000',
	...
	$st_options = get_option('st_options', array());
	$st_options = wp_parse_args($st_options, $default_options);

	$d = new uCal;
	$hoffset = intval(esc_html($st_options['hijri_adjust'])) * 60 * 60;
	...
	if ( !$st_options[ 'timetable' ] == '1' ) { // Automatic Timetable
		...
		if ( $st_options[ 'lang' ] == "bn" ) {
			...
			print( '<div class="salat-times-container"><div class="salat-times-widget"><table style="direction: ' . $st_options[ 'dir' ] . '; width: ' . $st_options[ 'width' ] . '; border-collapse: collapse;">
